...
export type TriggerDefinition = { type: 'manual' } | { type: 'schedule'; cron: string; timezone: string };
...
export interface RoutineInput {
  name: string;
  description?: string;
  trigger: { type: 'manual' } | { type: 'schedule'; cron: string; timezone?: string };
...
const TEMPLATE_ROOTS = new Set(['actions', 'routine', 'execution', 'now']);
...
export function validateRoutine(input: RoutineInput): RoutineDefinition {
  ...
  return { name: input.name.trim(), description: input.description?.trim() ?? '', trigger, actions };
}
